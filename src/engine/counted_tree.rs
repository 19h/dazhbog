...
fn decode(raw: &[u8]) -> sled::Result<(u64, u64)> {
    if raw.len() != 16 {
        return Err(sled::Error::Unsupported("invalid tree statistics; run preparation".into()));
    }
...
impl CountedTree {
        ...
        match stats.get(name)? {
            ...
            None => {
                ...
                for item in tree.iter() {
                    let (_, value) = item?;
                    count = count.checked_add(1).ok_or_else(|| io::Error::other("count overflow"))?;
                    bytes = bytes.checked_add(value.len() as u64).ok_or_else(|| io::Error::other("byte count overflow"))?;
                }
                stats.insert(name, encode(count, bytes))?;
            }
        }
        Ok(Self { tree, stats, name: name.to_vec() })
    }

    pub fn totals(&self) -> io::Result<(u64, u64)> {
        let raw = self.stats.get(&self.name)?.ok_or_else(|| io::Error::other("missing tree statistics"))?;
        Ok(decode(&raw)?)
    }

    pub fn insert<K: AsRef<[u8]>, V: Into<sled::IVec>>(&self, key: K, value: V) -> sled::Result<Option<sled::IVec>> {
    ...
    fn replace(&self, key: &[u8], value: Option<sled::IVec>) -> sled::Result<Option<sled::IVec>> {
        (&self.tree, &self.stats).transaction(|(tree, stats)| {
            let raw = stats.get(self.name.as_slice())?.ok_or_else(||
                ConflictableTransactionError::Abort(sled::Error::Unsupported("missing tree statistics".into())))?;
            let (count, bytes) = decode(&raw).map_err(ConflictableTransactionError::Abort)?;
            let old = tree.get(key)?;
            let count = count.checked_sub(u64::from(old.is_some()))
                .and_then(|n| n.checked_add(u64::from(value.is_some())));
            let bytes = bytes.checked_sub(old.as_ref().map_or(0, |v| v.len() as u64))
                .and_then(|n| n.checked_add(value.as_ref().map_or(0, |v| v.len() as u64)));
            let (Some(count), Some(bytes)) = (count, bytes) else {
                return Err(ConflictableTransactionError::Abort(sled::Error::Unsupported("tree statistics overflow".into())));
            };
            match &value {
            ...
            Ok(old)
        }).map_err(|e| match e { TransactionError::Abort(e) | TransactionError::Storage(e) => e })
    }
}
